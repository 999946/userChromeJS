...
window.UCL = {
	...
	init: function() {
		var opt = document.getElementById("addon-bar"); 
					...
					<menu label="新建|编辑样式">\
						<menupopup id="usercssloader-submenupopup">\
							...
							<menuitem label="userChrome.css"\
									  hidden="false"\
									  onclick="UCL.editUserCSS(event,\'userChrome.css\')" />\
							<menuitem label="userContent.css"\
									  hidden="false"\
									  onclick="UCL.editUserCSS(event,\'userContent.css\')" />\
						</menupopup>\
					</menu>\
					<menu label="设置|重载样式">\
						<menupopup>\
							...
							<menuitem label="设置外部编辑器"\
							          oncommand="UCL.setEditor();" />\
	...
	rebuildMenu: function(aLeafName) {
		...
		if (!menuitem) {
			...
			$("usercssloader-menupopup").appendChild(menuitem);
		}
	...
	itemClick: function(event) {
		...
		if (event.button == 1) {
			...
			UCL.edit(this.getFileFromLeafName(label));
		}
	...
	editUserCSS: function(event,aLeafName) {
...
function $(id) { return document.getElementById(id) || gNavToolbox.palette.querySelector('#'+id); }
